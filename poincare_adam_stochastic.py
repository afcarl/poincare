...
for ii in range(1):
    ...
    for jj in range(30000):
        ...
        for pos1 in vocab:
            ...
            update_der[pos1] = 1*der_p_emb0
            # update_der[pos2] = 1*der_p_emb1
        ...
        # print (loss_hist)
            for i in update_der:
                b1_t[i] *= b1
                b2_t[i] *= b2
                m[i] = b1*m[i] + (1.0 - b1) * update_der[i] 
                v[i] = b2*v[i] + (1.0 - b2) * update_der[i] * update_der[i] 
                update_der[i] =  alpha * (m[i] / (1.0 - b1_t[i])) / np.sqrt((v[i] / (1.0 - b2_t[i])) + eps);
                emb[i] = update(emb[i], update_der[i])
    # pre_emb = emb
    # plotall((ii+1) * 100000 + 456916)
